...
# use pony-script.py to create ctmp (The temp database)
...
#######################################################
PEOPLE=("Lawyer" "Plaintiff" "Witnes" "Surety" "Prosecutor" "Policeofficer" "Judicialofficer" "Defendant" "Visitor" "Warder")

# Reference tables
# Change these to suit your case
REFS=("Gender" "Causeofaction" "County" "Subcounty" "Constituency" "Court" "Courtlevel" "Doctemplate" "Lawfirm" "Natureofsuit" "Policerole" "Town" "Securityrank" "Policerank" "Warderrank" "Casecategory" "JoRank" "Paymentmethod" "Cell" "Prosecutorteam" "Prisonerproperty") #"Filingtype" "Commitaltype" "Policestationtype"


# Location Tables
# Places that require Lat,Lon,Alt
LOCS=("Prison" "Courtstation" "Court" "Investigation" "Policestation" "Lawfirm")


# Activity Tables
ACTS=("Visit" "Discipline" "Medevent" "Hearing" "Prisoncommital" "Case")
...
echo "# views.py AUTOGENERATED BY gen_script.sh from $1" >> kview.py
...
awk '{print "\
class "$0"View(ModelView):#MasterDetailView, MultipleView, CompactCRUDMixin\
...
echo "Generating ReportViews"

function gen_chartview(){
echo " " >> kview.py
echo "class $1ChartView(GroupByChartView):" >> kview.py
echo "    datamodel = SQLAInterface($1 , db.session)" >> kview.py
echo "    chart_title = 'Grouped $1 by Birth'" >> kview.py
echo "    chart_3d = 'true'" >> kview.py
echo "    label_columns = $1View.label_columns" >> kview.py
...
echo "class $1TimeChartView(GroupByChartView):" >> kview.py
echo "    datamodel = SQLAInterface($1 , db.session)" >> kview.py
echo " " >> kview.py
echo "    chart_title = 'Grouped Birth $1'" >> kview.py
...
echo "db.configure_mappers() #very important! for searchability" >>kview.py
...
echo -e "#make_versioned()\n$(cat kmodel.py)" > kmodel.py
echo -e "#make versioned copies of all tables\n$(cat kmodel.py)" > kmodel.py
echo -e "#force_auto_coercion()\n$(cat kmodel.py)" > kmodel.py
echo -e "# Initialize sqlalchemy_utils \n$(cat kmodel.py)" > kmodel.py
echo -e "import enum\n$(cat kmodel.py)" > kmodel.py
echo -e "import arrow, enum, datetime\n$(cat kmodel.py)" > kmodel.py
echo -e "\n# UTILITY CLASSES\n\n$(cat kmodel.py)" > kmodel.py
echo -e "\n$(cat kmodel.py)" > kmodel.py
...
echo -e "\t\t#def __declare_last__(cls):\n$(cat kmodel.py)" > kmodel.py
echo -e "\t\t#@classmethod\n$(cat kmodel.py)" > kmodel.py
echo -e "\n$(cat kmodel.py)" > kmodel.py
echo -e "# How to do validations in a model\n$(cat kmodel.py)" > kmodel.py
echo -e "from .mixins import *\n$(cat kmodel.py)" > kmodel.py
echo -e "from sqlalchemy.sql import func\n$(cat kmodel.py)" > kmodel.py
echo -e "from sqlalchemy_mixins import AllFeaturesMixin\n$(cat kmodel.py)" > kmodel.py
echo -e "from sqlalchemy.dialects.postgresql import *\n$(cat kmodel.py)" > kmodel.py
echo -e "from datetime import timedelta, datetime, date\n$(cat kmodel.py)" > kmodel.py
echo -e "\tUniqueConstraint,ForeignKeyConstraint, Numeric, LargeBinary , Table)\n$(cat kmodel.py)" > kmodel.py
echo -e "\tDateTime, Time, Boolean, Index, CheckConstraint,\n$(cat kmodel.py)" > kmodel.py
echo -e "\tSequence, Float, Text, BigInteger, Date,\n$(cat kmodel.py)" > kmodel.py
echo -e "from sqlalchemy import (Column, Integer, String, ForeignKey,\n$(cat kmodel.py)" > kmodel.py
echo -e "\n$(cat kmodel.py)" > kmodel.py
echo -e "from flask_validator import Validator\n$(cat kmodel.py)" > kmodel.py
echo -e "from sqlalchemy.dialects.postgresql import aggregate_order_by\n$(cat kmodel.py)" > kmodel.py
echo -e "    DATERANGE, TSRANGE, TSTZRANGE, TSVECTOR )\n$(cat kmodel.py)" > kmodel.py
echo -e "    TIME, TIMESTAMP, UUID, VARCHAR, INT4RANGE, INT8RANGE, NUMRANGE,  \n$(cat kmodel.py)" > kmodel.py
echo -e "    INTERVAL, JSON, JSONB, MACADDR, NUMERIC, OID, REAL, SMALLINT, TEXT,  \n$(cat kmodel.py)" > kmodel.py
echo -e "    DOUBLE_PRECISION, ENUM, FLOAT, HSTORE, INET, INTEGER,  \n$(cat kmodel.py)" > kmodel.py
echo -e "    ARRAY, BIGINT, BIT, BOOLEAN, BYTEA, CHAR, CIDR, DATE,  \n$(cat kmodel.py)" > kmodel.py
echo -e "from sqlalchemy.dialects.postgresql import ( \n$(cat kmodel.py)" > kmodel.py
echo -e "# IMPORT Postgresql Specific Types \n$(cat kmodel.py)" > kmodel.py


#echo -e "from flask_weasyprint import HTML, render_pdf\n$(cat kmodel.py)" > kmodel.py
echo -e "from sqlalchemy.orm import relationship, query, defer, deferred, column_property\n$(cat kmodel.py)" > kmodel.py
echo -e "from sqlalchemy_utils import aggregated, force_auto_coercion, observes\n$(cat kmodel.py)" > kmodel.py
...
echo -e "from sqlalchemy_continuum import make_versioned\n$(cat kmodel.py)" > kmodel.py
echo -e " \n$(cat kmodel.py)" > kmodel.py
...
do
#echo "s/class $i(/class $i(RefTypeMixin,  /g"
sed "s:class $i(:class $i(PlaceMixin, :g" k1.py > k2.py
cp k2.py k1.py
done

echo "Activity Mixins"
for i in "${ACTS[@]}"
do
#echo "s/class $i(/class $i(RefTypeMixin,  /g"
sed "s:class $i(:class $i(ActivityMixin, :g" k1.py > k2.py
cp k2.py k1.py
done
...
add_mixin Defendant PersonDocMixin
add_mixin Defendant PersonMedicalMixin
